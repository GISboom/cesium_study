...
function handleLeftDown(click) {
  ...
  //判断是否是多边形
  const polygon = allPolygons.find((item) => item.entity === entity);
  if (polygon) {
    ...
    //记录鼠标按下的位置
    const ray = viewer.camera.getPickRay(click.position);
    lastMousePosition = viewer.scene.globe.pick(ray, viewer.scene);
    enableCameraControl(false); //关闭相机控制
  }
}
...
//拖动多边形
function handlePolygonDrag(movement) {
  if(!isDraggingPolygon) return;
  const ray = viewer.camera.getPickRay(movement.endPosition);
  const currentPosition = viewer.scene.globe.pick(ray, viewer.scene);

  if (!currentPosition||!lastMousePosition) return;
  //计算偏移量
  const offset = Cesium.Cartesian3.subtract(currentPosition, lastMousePosition, new Cesium.Cartesian3());

  dragPolygon.positions = dragPolygon.positions.map((position) => {
